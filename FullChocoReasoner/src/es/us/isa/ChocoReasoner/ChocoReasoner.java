...
public class ChocoReasoner extends FeatureModelReasoner {
	...
	private HashMap<String,Constraint> configConstraints;
	...
	public void reset() {
		...
		this.configConstraints = new  HashMap<String,Constraint>();
...
//				new RandomIntVarSelector(null));
			...
			if (v instanceof GenericFeature) {
				...
				if (!features.containsKey(((GenericFeature) v).getName())) {
					...
					if (e.getValue() == 0) {
						...
						this.configConstraints.put("errorC",error);
						System.err.println("The feature " + v.getName()
								+ " do not exist, and can not be added");
					}
				} else {
					p.addConstraint(aux);
					this.configConstraints.put(v.getName()+" eq "+arg1,aux);
				}
	...
	public void unapplyStagedConfigurations() {
		Iterator<Constraint> it = this.configConstraints.values().iterator();
		Model p = this.getProblem();
		while (it.hasNext()) {
	...
	public HashMap<String, Constraint> getConfigConstraints() {
